            ...
            try:
                ...
                entry = {
                    ...
                    "thermodynamics": {
                        ...
                        "forward_length": fwd_thermo.length,
                        "forward_hairpin_dg": round(fwd_thermo.hairpin_dg, 2),
                        "forward_self_dimer_dg": round(fwd_thermo.self_dimer_dg, 2),
                        "forward_gc_clamp": fwd[-1].upper() in ("G","C"),
                        ...
                        "reverse_length": rev_thermo["length"] if rev_thermo else None,
                        "reverse_hairpin_dg": round(rev_thermo["hairpin_dg"], 2) if rev_thermo else None,
                        "reverse_self_dimer_dg": round(rev_thermo["self_dimer_dg"], 2) if rev_thermo else None,
                        "reverse_gc_clamp": rev[-1].upper() in ("G","C") if rev else None,
                        "quality_score": thermo_score,
                        "tm_delta": pair_data.get("tm_delta") if pair_data else None,
                        "cross_dimer_dg": pair_data.get("cross_dimer_dg") if pair_data else None,
                        "annealing_temp": pair_data.get("annealing_temp_suggested") if pair_data else None,
                    },
